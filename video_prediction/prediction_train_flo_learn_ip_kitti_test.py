...
def get_image_grad(image):
  ...
  return tf.concat([image, 
                    image - image_pad[:, 1:-1, 0:-2, :], 
                    image - image_pad[:, 1:-1, 2:, :],
                    image - image_pad[:, 0:-2, 1:-1, :],
                    image - image_pad[:, 2:, 1:-1, :], 
                    image - image_pad[:, 0:-2, 0:-2, :],
                    image - image_pad[:, 0:-2, 2:, :],
...
#         im.save(os.path.join(FLAGS.output_dir, file_name[i][0]+"_11.jpeg"))
        ...
        flo_true = read_flow_png(os.path.join("/home/wangyang59/Data/data_scene_flow/training/flow_occ", file_name[i][0]+"_10.png"))
        flo_true_noc =  read_flow_png(os.path.join("/home/wangyang59/Data/data_scene_flow/training/flow_noc", file_name[i][0]+"_10.png"))
         
        occu_mask_true = 1.0 - (flo_true[:, :, 2:] - flo_true_noc[:, :, 2:])
        occu_mask_pred = np.expand_dims(cv2.resize(occu_mask[i], (orig_w, orig_h), interpolation=cv2.INTER_CUBIC), axis=-1)
         
        img1 = cv2.resize(image1[i], (orig_w, orig_h), interpolation=cv2.INTER_CUBIC)
        img2 = cv2.resize(image2[i], (orig_w, orig_h), interpolation=cv2.INTER_CUBIC)
         
        ims1 = plot_general([np.expand_dims(img1, axis=0), 
                             np.expand_dims(flo_true[:, :, 0:2], axis=0), np.expand_dims(flo_pred[:, :, 0:2], axis=0), 
                             np.expand_dims(occu_mask_true, axis=0), np.expand_dims(occu_mask_pred, axis=0)], h=1, w=5, output_dir=FLAGS.output_dir, itr=itr, get_im=True)
         
        ims1[0].save(os.path.join(FLAGS.output_dir, file_name[i][0]+".png"))
