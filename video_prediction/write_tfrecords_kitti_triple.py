...
def convert_to(input_tuple):
  ...
  for file_name in file_names:
    ...
    img3 = cv2.imread(image3_file)
     
    img_all = np.concatenate((img, img2, img3), axis=0)
    img_all_convert = hisEqulColor(img_all)
    img_convert, img2_convert, img3_convert = np.vsplit(img_all_convert, 3)
    
    cv2.imwrite("/home/wangyang59/convert1.jpeg", img_convert)
    cv2.imwrite("/home/wangyang59/convert2.jpeg", img2_convert)
    cv2.imwrite("/home/wangyang59/convert3.jpeg", img3_convert)
    ...
    im = Image.open("/home/wangyang59/convert3.jpeg")
    im = im.crop((0, 0, 1224, 370))
    output = StringIO.StringIO()
    im.save(output, format="jpeg")
...
def main(unused_argv):
  ...
  for scene in scenes:
    for i in range(len(frames)-2):
      if frames[i] not in ["08", "09", "10", "11", "12"]:
        for seg in ["training", "testing"]:
          for image in ["image_2", "image_3"]:
            ...
            file3 = os.path.join(data_dir, seg, image, scene+"_"+ frames[i+2] + ".png")
            ...
            if os.path.exists(file1) and os.path.exists(file2) and os.path.exists(file3):
              training_data.append([file1, file2, file3])
  ...
  for i in range(n/batch_size + 1):
    output_file = "/home/wangyang59/Data/ILSVRC2016_tf_kitti_2012_train_hist_triple/%s.tfrecord" % i
