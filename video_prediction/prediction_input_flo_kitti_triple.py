...
def build_tfrecord_input(training=True, num_epochs=None):
  ...
  """
  train_2012_filenames = gfile.Glob(os.path.join("/home/wangyang59/Data/ILSVRC2016_tf_kitti_2012_train_hist_triple", '*'))
  ...
  if training:
    filenames = train_2012_filenames
  else:
  ...
  if training:
    ...
    image3 = tf.reshape(image3, [1, ORIGINAL_HEIGHT, ORIGINAL_WIDTH, COLOR_CHAN])
    image3 = tf.cast(image3, tf.float32) /255.0
    image3 = tf.image.resize_bicubic(image3, [RESIZE_HEIGHT, RESIZE_WIDTH])
    image3 = tf.reshape(image3, [RESIZE_HEIGHT, RESIZE_WIDTH, COLOR_CHAN])
  else:
  ...
  if training:
    images = tf.concat([image1, image2, image3], axis=2)
    images = tf.image.random_flip_left_right(images)
    #images = tf.image.random_flip_up_down(images)
    #images = tf.cond(tf.random_uniform([]) < 0.5, lambda: tf.image.rot90(images, 2), lambda: images)
    images.set_shape([RESIZE_HEIGHT, RESIZE_WIDTH, COLOR_CHAN*3])    
    image1, image2, image3 =  tf.split(axis=2, num_or_size_splits=3, value=images)
    
    image1, image2, image3 = tf.cond(tf.random_uniform([]) < 0.5, lambda: [image1, image2, image3], lambda: [image3, image2, image1])
